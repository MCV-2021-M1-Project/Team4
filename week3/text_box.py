...
def bounding_box(img,mask = None):
    ...
    mask_components = cv2.connectedComponentsWithStats(mask.astype(np.uint8)) # We take picture's masks' right and left coordinates
    ...
    expanded_mask = np.zeros((height + 60, width + 22)) # We create an expanded mask to avoid problems in edges with morphological filters
    ...
    expanded_mask = cv2.morphologyEx(expanded_mask, cv2.MORPH_OPEN, np.ones((3,4)))   # Delete vertical lines (kernel = (1,4)) and horizontal lines (kernel = (4,1)). Total kernel = (4,3)
    ...
    ### 2. ITERATION ###
    ## STEP 1. Blackhat and thresholding
    expanded_mask = np.zeros((height, width + 200))                    # We create an expanded mask to avoid problems in edges with morphological filters
    expanded_mask = blackhat_box > 0.4

    ## STEP 2. Morphological filters
    expanded_mask = expanded_mask.astype(np.uint8)
    expanded_mask = cv2.morphologyEx(expanded_mask, cv2.MORPH_CLOSE, np.ones((5,14))) # Fill letters
    expanded_mask = cv2.morphologyEx(expanded_mask, cv2.MORPH_OPEN, np.ones((4,4)))   # Delete vertical lines (kernel = (1,4)) and horizontal lines (kernel = (4,1)). Total kernel = (4,3)
    ...
    central_line = mask[top + int((bottom - top)/2),:]      # We take the row of the mask that lies in the middle of the bounding box
    ...
    mean_distance = int((distance_to_text_left + distance_to_text_right)/2) # Mean left and right distance from bounding box to the mask
...
def biggest_component(mask):
    ...
    if components[0] > 1: # If we have detected at least 1 connected component
        ...
        not_middle = not (top_cc < int(height/2) and top_cc + height_cc > int(height/2)) # The component must be in the upper or bottom region of the mask, not in both
