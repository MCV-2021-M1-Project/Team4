...
def main():
    ...
    with open(args.q / 'frames.pkl', 'rb') as f:
    ...
    for i in tqdm(range(n_query_images)): # range(n_query_images)
        ...
        angularError.append(angular_error(frames[i][0][0], angle))
        ...
        for idx, mask in enumerate(masks):
            ...
            image_distances[idx] = check_painting_db(distances_i, image_distances[idx], type=args.f) #sift 0.03, orb 0.24
    ...
    print()
    print(f'Mean Angular Error:')
    print(f"{sum(angularError)/len(angularError)}:.04f")
